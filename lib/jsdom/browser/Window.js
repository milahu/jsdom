...
const CSSPseudoElementImpl = require("../living/nodes/CSSPseudoElement-impl").implementation;
...
function Window(options) {
  ...
  this.getComputedStyle = function (elt, pseudoElt = null) {
    ...
    if (pseudoElt !== undefined && pseudoElt !== null && pseudoElt !== "") {
      ...
      // pseudoElt: with colons: ::before, :before
      // pseudoId: no colons: before
      const pseudoId = pseudoElt.replace(/^::?/, '');
      if (!CSSPseudoElementImpl._pseudoIdList.includes(pseudoId))
        throw new TypeError("Tried to get the computed style of invalid pseudo-element.");
      elt = elt._createPseudoElement(pseudoId);
    }
